...
if uploaded_file:
    ...
    if 'df' not in st.session_state:
        ...
        # 라벨된 마지막 인덱스 다음 줄부터 시작
        labeled_indices = df[df['label'].notna()].index
        if not labeled_indices.empty:
            last_labeled_idx = labeled_indices.max()
            next_idx = last_labeled_idx + 1 if last_labeled_idx + 1 < len(df) else last_labeled_idx
        else:
            next_idx = 0
